...
///
/// デバイスオフセット (#85, #99):
///   トラッカーの原点は実際に持つデバイス (食品モデル) の中心とは一致しない
///   (ベルト/マウント分ずれる上、モデルごとにずれ方も違う)。
///   positionOffset / rotationOffset に「トラッカーローカル空間でのずれ」を入れると、
///   追従先をデバイス実体の位置・向きへ補正する。問題・選択肢ごとに別の食品モデル
///   オブジェクトが GameFlow prefab にそれぞれ存在するため、本コンポーネントの
///   インスタンスごとに Inspector で個別に設定する (共有アセットにはしない)。
/// </summary>
public class ViveTrackerFollower : MonoBehaviour
{
    ...
    [Header("デバイスオフセット (#85, #99)")]
    [Tooltip("トラッカー原点 → このモデル実体への位置ずれ (トラッカーローカル空間, m)")]
    ...
    /// Track 出来ていなければ false。距離判定などで「デバイス実体の位置」を
    /// 使いたい場合の窓口 (本コンポーネント自身は LateUpdate で同じ計算を適用済み)。
    /// </summary>
